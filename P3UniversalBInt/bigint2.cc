...
/**
 * @brief Pow
 * Algoritm: https://en.wikipedia.org/wiki/Exponentiation_by_squaring
 * @param first 
 * @param other 
 * @return BigInt<2> 
 */
...
BigInt<base> BigInt<2>::ConvertBaseT(){
  std::cout << "Bigint2.cc ConvertBaseT" << std::endl;
...
/**
 * @brief Overload the operator >> to read a BigInt<2> object
 *
 * @param is
 * @param number_x
 * @return std::istream&
...
// Methods from class Number

Number* BigInt<2>::add(const Number* otherNum) const {
  // Primero, verificamos que el otro número sea del mismo tipo que nuestro BigInt.
  const BigInt* other = static_cast<const BigInt*>(otherNum);
    if (other == nullptr) {
        throw std::invalid_argument("The other number is not a BigInt");
    }
...
/**
 * @brief Overide the write method from Number class
 * 
 * @return std::ostream& 
 */
...
/**
 * @brief Overide the read method from Number class
 * 
 * @return std::istream& 
 */
